...
export default function TradingTerminal() {
  ...
  const generateAdvancedIndicators = () => {
    const indicators: AdvancedIndicator[] = [
      ...
      },
      {
        ...
        signal: Math.random() > 0.4 ? 'sell' : 'buy',
  ...
  const generateAIPrediction = () => {
    const predictions = [
      { direction: 'bullish', probability: 0.72, target: '+5.2%', timeframe: '24h' },
      { direction: 'bearish', probability: 0.68, target: '-3.1%', timeframe: '24h' },
      { direction: 'neutral', probability: 0.61, target: '±1.5%', timeframe: '24h' }
    ];
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 space-y-3 sm:space-y-0">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 w-full sm:w-auto justify-between sm:justify-end">
              <div className="text-lg sm:text-2xl font-bold text-white">
                {candlestickData.length > 0 ? candlestickData[candlestickData.length - 1].close.toFixed(2) : '---'}
              </div>
              <div className={`flex items-center space-x-1 ${
                candlestickData.length > 0 && candlestickData[candlestickData.length - 1].close > candlestickData[candlestickData.length - 1].open
