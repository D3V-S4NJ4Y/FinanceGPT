    ...
    try {
      ...
      if (predictionResults.some(r => r && r.prediction)) {
        ...
        symbols.forEach((symbol, index) => {
          ...
          newPredictions[symbol] = mockPrediction;
          
          if (mockPrediction.confidence > 0.8) {
            addAlert({
              type: 'prediction',
              severity: mockPrediction.risk_score > 0.6 ? 'high' : 'medium',
              message: `High confidence ${mockPrediction.direction} signal for ${symbol}`,
              symbol: symbol,
              action: mockPrediction.direction === 'bullish' ? 'Consider buying' : 'Consider selling'
            });
          }
        });
      }
