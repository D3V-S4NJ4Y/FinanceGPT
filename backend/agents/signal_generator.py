...
class SignalGeneratorAgent(BaseAgent):
    ...
    async def _get_real_market_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if latest_data and len(latest_data) > 0:
                ...
                return {
                    "current_price": float(record[1]) if record[1] else 100.0,  # price
                    "volume": int(record[2]) if record[2] else 1000000,          # volume
                    "change_percent": float(record[4]) if record[4] else 0.0,    # change_percent
                    "change": float(record[3]) if record[3] else 0.0,            # change
                    "symbol": record[0] if record[0] else symbol                 # symbol
                }
            else:
            ...
            logger.error(f"❌ Portfolio signal generation error: {e}")
            return {"error": str(e)}
    ...
    async def process_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """
        🔄 Process incoming trading signal requests
        """
        try:
            message_type = message.get('type', 'generate_signals')
            
            if message_type == 'generate_signals':
                symbols = message.get('symbols', ['AAPL', 'GOOGL', 'MSFT'])
                signals = {}
                
                for symbol in symbols:
                    # Get market data for symbol (in real implementation, fetch from data source)
                    market_data = {"symbol": symbol, "price": 100.0, "volume": 1000000}
                ...
                market_data = {"symbol": symbol, "price": 100.0, "volume": 1000000}
                analysis = await self._analyze_technical_indicators(symbol, market_data)
                ...
                # Default - generate signals for popular stocks
                signals = {}
            ...
            logger.error(f"❌ Signal generator processing error: {e}")
            return {
                'agent': 'SignalGenerator',
                'type': 'error',
                'timestamp': datetime.now().isoformat(),
